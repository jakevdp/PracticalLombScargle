...
\begin{description}
   ...
   \item[A time-shift imparts a phase in the Fourier transform.]
     That is for any function $g(t)$,
     \begin{equation}
       \mathcal{F}\{g(t - t_0)\} = e^{-2\pi i ft_0}\mathcal{F}\{g(t)\}
     \end{equation}
     The key observation here is that the time-shift does not change the
     amplitude of the result, but only the phase.
\end{description}

There are a couple useful consequences of these properties.
...
\caption{Visualization of important Fourier pairs.}
\end{figure}

Dirac comb
\begin{equation}
    C(t, \Delta t) = \sum_{k=-\infty}^\infty \delta(t - k\Delta t)
\end{equation}
Fourier transform is
\begin{equation}
    \mathcal{F}\{C(t, \Delta t)\} = \frac{1}{\Delta t} C(f, 1/\Delta t)
\end{equation}


Tophat and Sinc
\begin{equation}
  tophat(t; T) \equiv \left\{
  \begin{array}{ll}
    1 / T & {\rm if}\ |t| < T / 2 \\
    0     & {\rm otherwise}
  \end{array}
...
Fourier transform is
\begin{equation}
  sinc(t) \equiv \frac{\sin(\pi t)}{\pi t}
\end{equation}
