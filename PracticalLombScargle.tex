...
\cfoot{\it \small Draft Version \today}
...
\begin{description}
   ...
   \item[The Fourier transform of sinusoid with frequency $f_0$ is a sum of delta functions at $\pm f_0$.]
     From the integral definition of the Dirac delta function, we can write
     \begin{equation}
       \mathcal{F}\{e^{2\pi f_0 t}\} = \delta(f - f_0).
       \eqlabel{delta-FT}
     \end{equation}
     Using Euler's formula for the complex exponential along with the linearity
...
Notice, though, that we glossed over one thing: the effect of switching
...
will generally {\it not} be a clean sequence of delta functions; the
...
The resulting periodogram, then, can only be used as an approximation
of the true periodogram, and will only approach the true periodogram as the
number of observed samples becomes very large.
...
\begin{eqnarray}
  \sigma_f &\approx& f_{1/2} / \sqrt{P_{max}}\nonumber\\
           &\approx& f_{1/2} \sqrt{\frac{2}{N\Sigma^2}}.
  \eqlabel{bayes-error}
\end{eqnarray}
