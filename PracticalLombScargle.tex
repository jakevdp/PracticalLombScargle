...
After considering all of these practical aspects, I think it is worth
revisiting the question of {\it why} astronomers lean toward the
Lomb-Scargle approach rather than the (in many ways simpler) Schuster
periodogram.
In particular, after three decades of use, there seems to be an unrealistic
mythology about the efficacy of the Lomb-Scargle approach among astronomers.
...
\begin{itemize}
  ...
  \item {\it Myth: The Lomb-Scargle periodogram is faster than the least squares
    periodogram because it avoids explicitly solving for model coefficients.}
  ...
  \item {\it Myth: The Lomb-Scargle periodogram corrects for aliasing due to
    ...
    periodogram (see \sect{failure-modes}).
  \item {\it Myth: Bayesian analysis shows that Lomb-Scargle is the optimal
    statistic for detecting periodic signals in data.} Reality: it rather
...
What advantages would we lose by instead using a Schuster periodogram,
...
data.
The point of correspondance for a sinusoidal model is perhaps useful trivia,
but not a reason to use that model if your data is not known to be sinusoidal.

Conversely, if you use a Schuster approach rather than a Lomb-Scargle approach,
...
While the deconvolution problem is well-known to be ill-posed, there may be a
way forward for cleaned periodograms based on regularized deconvolutions.
