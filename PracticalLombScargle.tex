...
From \citet{Eyer99}: let $p$ be the largest value such that each $t_i$ can be written $t_i = t_0 + n_i p$, for integers $n_i$. The Nyquist Frequency then is:
\begin{equation}
  \nu_{Ny} = 1 / (2p)
\end{equation}

Examples of wrong approaches: average \citep{NumRec, Horne86, Scargle82}, minimum \citep{Roberts87, Press89}, average of inverse time intervals \citep{Debosscher07},
...
\begin{figure}[ht]
  ...
  \caption{An example of data for which the various poorly-motivated
    ...
    the minimum spacing is 2.5, meaning that the signal has
    {\it over 250 full cycles} between the minimum spacing.
    Nevertheless, the periodogram (lower right) clearly identifies the correct
    period, though it's orders of magnitude larger than Nyquist estimates based
    on average or minimum sampling rate.
