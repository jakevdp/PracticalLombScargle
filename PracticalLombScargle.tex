...
Examples: average \citep{NumRec, Horne86}, minimum \citep{Scargle82, Roberts87}, average of inverse time intervals \citep{Debosscher07}.
...
\citet{Roberts87}: ``for arbitrary $\{t_r\}$,
the sampling theorem tells us nothing.
If the data samples are otherwise equally spaced but with missing
points, the theorem says thatthe data completely determine
...
\citet{Horne86}: ``The largest frequency we calculated was $\pi N-)/T$ which
...
\citet{Debosscher07}: ``For the
highest frequency, we used the average of the inverse time intervals
