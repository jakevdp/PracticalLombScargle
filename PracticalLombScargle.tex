...
more carefully; see, e.g., \citep{Vio2010} or the Least Squares approach
...
This also suggests a convenient way to construct a periodogram that accounts
for correlated noise across observations; denoting the  $N\times N$ noise
covariance matrix for $N$ observations by $\Sigma$, and constructing the
vectors $\vec{y} = [y_1, y_2,\cdots y_n]^T$
and $\vec{y}_{model} = [y_{model}(t_1),y_{model}(t_1),\cdots y_{model}(t_n)]^T$,
then the $\chi^2$ statistic can be written
\begin{equation}
  \chi^2(f) = (\vec{y}-\vec{y}_{model})^T\Sigma^{-1}(\vec{y}-\vec{y}_{model})
  \eqlabel{chi2-with-correlated-errors}
\end{equation}
In the case of uncorrelated errors, the off-diagonal terms of $\Sigma$
are zero, and this expression reduces to \eq{chi2-with-errors}.
This is similar in spirit to the approach to correlated noise developed
by \citet{Vio2010} from the Fourier perspective, and is in fact identical
in the case of the ``zero-mean colored noise'' example considered therein.
...
with that in mind one quick way to compute a qualitatively correct window
power is to use a standard Lomb-Scargle periodogram on a series of unit
measurements, making sure to {\it not} pre-center the data or to use a
floating mean model.
As \citep{Scargle82} notes, this is {\it not} the true window---it differs
