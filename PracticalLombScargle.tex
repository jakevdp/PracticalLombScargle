...
For the standard model, the Bayesian periodogram is given by
\begin{equation}
  p(f~|~D) \propto e^{-N \chi^2 / 2} \propto e^{P_{LS}(f)}
...
a simple probability of the data showing a particular frequency, but rather
...
but that appeal extends to the real world only in special cases.
That is not to say that all Bayesian approaches to periodic analysis are
...
extensions with strong priors \citep[e.g.][]{Bretthorst88}, flexible
non-parametric functions \citep[e.g.][]{Gregory92}, constrained stochastic
models \citep[e.g.][]{Kelly14}, and Gaussian process models \citep{Angus16}.
...
\begin{figure}[ht]
  ...
  \caption{Comparison of the true period and peak Lomb-Scargle period for
    ...
    Panels (a)-(d) show some of the failure modes we can expect due to this
    window function; see the text for more discussion.
...
\fig{failure-modes}; the periods shown are
\begin{equation}
  P_{obs} = \left(\frac{1}{P_{true}} + n\delta f\right)^{-1}
\end{equation}
and for this data, close to 36\% of the objects are mis-characterized this way.
...
This can occur for signals that are not strictly sinusoidal, and this harmonic
frequency peak is subject to the same windowing effects as above.
Thus we can describe the failure modes depicted in panel (b) of \fig{failure-modes} with the following equation, for integers $m > 0$:
\begin{equation}
...
Lomb-Scarlge results.
This is closely related to panels (a) and (b), but comes from the fact that
...
1\% of points are misclassified in an ``unexplained'' way, as seen in panel (d).
...
All code for this paper is available in the form of Jupyter notebooks in the
