...
\begin{abstract}
This paper gives an introduction to the practical aspects of the use of
Lomb-Scargle periodograms and related algorithms to detect and characterize 
periodicity within a signal.
Rather than a rigorous mathematical treatment, the goal of this paper is to
build intuition about what assumptions are baked into any periodogram
computation and to help develop the intuition required to interpret the
...
clean, well-defined procedure to detect the periodic component in an
unevenly-sampled dataset.
...
observation patterns (i.e. window functions) affect the power spectrum, and
discuss a conceptual understanding of the well-known Nyquist limiting frequency.
In \sect{non-uniform-sampling}, we consider non-uniformly sampled signals as
a special case of such windowing, and show that the Nyquist-like limit for
this case is quite different than what many practitioners in the field
assume.
In \sect{schuster-to-lomb-scargle}, we discuss the properties of the
Lomb-Scargle periodogram, a modified version of the classical periodogram
for unevenly-sampled data, as well as the motivation behind these modifications.
In \sect{lomb-scargle-extensions}, we discuss a complementary view of the
periodogram as the result of least-squares model fitting, as well as some
extensions that allows (including the ``Bayesian'' periodogram).
...
the choice of frequency grid, false alarm probabilities, and various modes
of failure that should be expected when working with real-world data.
Finally in \sect{conclusion}, we conclude and summarize the key recommendations
for practical use of the Lomb-Scargle periodogram.

\subsection{Code and Figures}
...
the total signal.
Note that if $g$ is real-valued, it can be shown that
$\mathcal{P}_g(f) = \mathcal{P}_g(-f)$.
...
For our purposes, this property of the transform will turn out to be quite
...
Here we consider a periodic function observed only within the window $-3<t<3$.
The observed signal in this case can be thought of as a pointwise product of
...
this function is equivalent to the convolution of the Fourier transforms of
each component, which are a set of delta functions and a sinc function,
respectively.
For a purely periodic signal, this convolution has the effect of broadening
the delta functions representing the signal, turning each of them into a
...
the true Fourier transform values are non-zero only within a range of width $1/T$.
...
A sparser sampling of the signal leads to a narrower sampling in frequency,
and the signal transform no longer fits within the frequency window.
The result is that the true Fourier transform {\it cannot be recovered}
...
\end{equation}
Apart from the $1/N$ scaling, this sum is precisely the Fourier power
...
\begin{equation}
  ...
  \eqlabel{g-obs-conv}
\end{equation}
Unlike in the uniform case, the window transform $\mathcal{F}\{W\}$ will
generally {\it not} be a nice clean function; from \eq{FT-def} and
\eq{nonuniform-window}, we can write
\begin{equation}
  \mathcal{F}\{W\}(f) = \sum_{n=1}^{N} e^{-2\pi i f t_n}
...
senses it is: the locations and heights of the Fourier peaks are related to
...
For a discussion of nonuniform window functions and attempts to correct
for their effect on the periodogram, see \sect{windows-and-deconvolution}.
...
of the Dirac comb window function created by evenly-sampled data,
...
Quantitatively, if the times are recorded to $D$ decimal places,
the Nyquist frequency will be at most
\begin{equation}
  f_{Ny} = \frac{1}{2} 10^D,
  \eqlabel{nonuniform-nyquist}
\end{equation}
(cf. \fig{nyquist-eyer99}).
...
\subsubsection{Which Limiting Frequency is Useful?}

\todo{Computational constraints; use of prior knowledge}
...
\todo{Mention requirement of uncorrelated noise, and correction for this explored by \citet{Vio2010}}
...
\citet{Scargle89}, and \citet{Irwin89};
\citet{Zechmeister09} showed that such a modification
...
knowledge of your data to decide when it indicates a need for a more
complicated model, and additionally what form of additional variation to add.
...
On the astronomy side, several examples of such extensions exist.
...
delta-function representation of the window
function; recalling \eq{nonuniform-window-FT}, the window power spectrum
...
with that in mind we can compute the Lomb-Scargle window power by computing
a standard Lomb-Scargle periodogram on a series of unit measurements,
making sure to {\it not} pre-center the data or use a floating mean model.
...
\end{equation}
Given this direct relationship, we might hope to be able to invert this
...
values of $t$ and such an inversion is not well-defined.
More precisely, it can be shown that deconvolution in this context is an
ill-posed problem without a unique solution, and so direct deconvolution
is not an option.
...
One point of confusion in periodogram implementations is the interpretation
of the periodogram values and their meaning.
...
Each of these suggests a different natural approach to normalizing and interpreting the periodogram.
