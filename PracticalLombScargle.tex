...
\citet{Scargle82} shows that the classical periodogram of
\eq{classical-periodogram} has such desireable statistical properties in the
case of uniform sampling, but not in the case of non-uniform sampling.
Such desireable properties can be recovered in the non-uniform case by
applying a slight modification to the standard periodogram:
\begin{equation}
  P(f)
  = \frac{1}{2}\left[
    \frac{\left[\sum_n g_n \cos(2\pi f [t_n-\tau])\right]^2}
         {\sum_n g_n \cos^2(2\pi f [t_n-\tau])}
  + \frac{\left[\sum_n g_n \sin(2\pi f [t_n-\tau])\right]^2}
         {\sum_n g_n \sin^2(2\pi f [t_n-\tau])}
  \right]
  \eqlabel{lomb-scargle-periodogram}
\end{equation}
where $\tau$ is given by
\begin{equation}
  \tan(4\pi f \tau) = \frac{\sum_n 4\pi f \tau}{\sum_n 4\pi f\tau}.
  \eqlabel{tau-def}
\end{equation}
This form has since become commonly referred to as the ``Lomb-Scargle Periodogram'', after \citet{Lomb76} and \citet{Scargle82} who proposed this alternate
periodogram and explored its statistical properties.
