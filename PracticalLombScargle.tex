...
periodogram, so as to motivate important practical considerations required
...
the Fourier transform directly measures periodic content in a continuous function.
...
{\it power spectral density} or the {\it power spectrum}:
\begin{equation}
...
Lomb-Scargle we will discuss momentarily)
...
of observations
\citep[for a detailed discussion, see Chp 8.4 of][]{Anderson1971}.
...
\begin{enumerate}
  ...
  \item The periodogram's statistics are analytically computable,
...
construct it such that it correctly handles measurement error in the data.
...
we can re-write \eq{chi2-simple} in the following (standard) way:
\begin{equation}
...
{\it in prep}\footnote{\url{http://ascl.net/code/v/1559}})
...
over a classical periodogram?
What would we lose if we instead used a simple Fourier periodogram,
...
Foreman-Mackey, and Zeljko Ivezic for helpful feedback
